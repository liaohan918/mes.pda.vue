...
$(function () {
	...
	$('#txtBarcode1').keydown(function (event) {
		...
		if (event.keyCode == '13' && barcode != '') {
			//请求服务，拿物料编码、供应商、批次、客户编号，并写到输入框内
			//           DAB020、DAB019、DAB007、DAB030
			$.get(app.API_URL_HEADER + '/BARDAH/GetBarcodeInfo', { barcode: barcode }, function (result) {
	...
	$('#btnQuery').click(function () {
		...
		//TODO判断物料编码是否为空
		$.get(app.API_URL_HEADER + '/BARDAH/QueryFrozenMaterials',
			{ materialNo: $('#DAB020').val(), supplierNo: $('#DAB019').val(), batchNo: $('#DAB007').val(), customerNo: $('#DAB030').val() }
			, function (result) {
				var data = $.parseJSON(result.data)
				if (data.length == 0) {
					mui.toast('无此物料')
	...
	//批量解冻按钮事件
	$('#btnUnFreeze1').click(function () {
		console.log('unfreeze')
		var checkedRowsBarcodes = []
		var checkedRows = $('#datagrid').datagrid('getChecked')
		for (var i = 0; i < checkedRows.length; i++) {
			checkedRowsBarcodes.push(checkedRows[i].DAH002)
		}
		console.log(checkedRowsBarcodes)
		var currentSession = app.loadconfig(app.CONFIG_CURRENT_SESSION);
		var user_id = currentSession.user_id;
		$.post(app.API_URL_HEADER + '/BARDAH/UnFreezeBatch', { barcodes: checkedRowsBarcodes, userid: user_id }, function (result) {
			if (result.status == 0) {
				mui.toast("解冻成功!");
				playerAudio("OK")
			} else {
				mui.toast("解冻失败!"+result.message);
				playerAudio("NG")
			}
		})
	})
	...
	//单个解冻按钮事件
	$('#btnUnFreeze2').click(function () {
		var barcode = $('#txtBarcode2').val()
		console.log(barcode)
		var currentSession = app.loadconfig(app.CONFIG_CURRENT_SESSION);
		var user_id = currentSession.user_id;
		$.post(app.API_URL_HEADER + '/BARDAH/UnFreezeSingle', { barcode: barcode, userid: user_id }, function (result) {
			if (result.status == 0) {
				mui.toast("解冻成功!");
				playerAudio("OK")
			} else {

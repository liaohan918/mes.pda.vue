...
$(function() {
	...
	//获取未完成的盘点单号
	$.ajax({
		url: app.API_URL_HEADER + '/PanDian/GetNoPingZhangPDDanHao',
		data: {},
		dataType: "json",
		type: "post",
	...
	$('#txtBarcode').keydown(function(e) {
		...
		$.ajax({
			url: app.API_URL_HEADER + '/PanDian/GetFuPanTiaoMaMSG',
			data: {
				...
				if(res.status == 0) {
					...
					if(dt1.length > 0) {
						if(dt1[0]["CKBM"] == "WDJ") {
							...
							if(dt1[0]["FPR"] == "" ||
								((dt1[0]["FPR"] != "") && confirm("条码[" + barCode + "]已盘点，是否重盘？"))) {
...
function GetData(paging, pageSize, pageNumber, keys) {
	$.ajax({
		url: app.API_URL_HEADER + '/PanDian/GetFuPanDataByPDDanHao', //获取数据:调用webApi服务路径
		data: {
...
function UpdatePD() {
	...
	$.ajax({
		url: app.API_URL_HEADER + '/PanDian/UpdateFuPan',
		data: {
			PDDH: pddh,
			isNew: isNewBarcode,
			Barcode: barcode,

...
//		var user_id = "admin";
 		$.ajax({
 			url: app.API_URL_HEADER + "/WARBAB/GetBarInfo",
 			...
 			success: function(data) {
 				...
 				if(data.status != 0) {
                    ...
                    $("#info").val("");//先清空一下数据源
 					...
 					//如果当前库位为空或者服务器传回来的库位为空，则取消勾选
 					if($("#MBA001").val() == "" || data.data.lastStore == "") {
// 						$("#checkdefault").prop("checked", false); //暂时取消这个限制，方便5楼在库入库，五楼入完取消
 					}
 					//判断当前库位与服务器传回来的库位是否相等，不相等则取消勾选框
 					if($("#MBA001").val() != data.data.lastStore) {
// 						$("#checkdefault").prop("checked", false); //暂时取消这个限制，方便5楼在库入库，五楼入完取消
// 						$("#MBA001").value = data.data.lastStore;//将上一次的库位带出来
 					}
 					//带出库位

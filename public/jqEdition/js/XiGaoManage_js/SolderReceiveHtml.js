...
$(function() {
	...
	$('#txtBarcode').keydown(function(e) {
		...
		$.ajax({
			url: app.API_URL_HEADER + '/XiGao/GetLingYongBarcodeMsg',
			data: {
				Barcode: barCode
			},
			dataType: "json",
				...
				if(res.status == 0) {
					...
					if(dt1.length > 0) {
						if(dt1[0]["YiTime"] >= dt1[0]["XuTime"] ) {
							...
							msg = msg + "开始搅拌时间：" + dt1[0]["KaiShiTime"] + "\n";
							msg = msg + "已搅拌时间(分钟)：" + dt1[0]["YiTime"] + "\n";
							msg = msg + "需搅拌时间(分钟)：" + dt1[0]["XuTime"] + "\n";
							$('#info').val(msg);
						} else {
							playerAudio('NG');
							mui.alert('此条码搅拌时间不够，不能进行领用！');
							$('#info').val('');
...
function GetData(paging, pageSize, pageNumber, keys) {
	$.ajax({
		url: app.API_URL_HEADER + '/XiGao/GetJiaoBanXiGao', //获取数据:调用webApi服务路径
		data: {
			paging: paging, //是否分页
...
//var user_id ='noid';
	$.ajax({
		url: app.API_URL_HEADER + '/XiGao/UpdateLingYong',
		data: {
			Line:line,
			Barcode: barcode,
			LoginUser: user_id
